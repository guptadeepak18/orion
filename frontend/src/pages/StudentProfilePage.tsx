...
export const StudentProfilePage: React.FC = () => {
  ...
  // Countdown timer for password change OTP resend
  React.useEffect(() => {
    if (pwdResendTimer > 0) {
      const t = setTimeout(() => setPwdResendTimer(pwdResendTimer - 1), 1000);
  ...
  const handleRequestPasswordOtp = async () => {
    ...
    try {
      ...
      setPwdStep(2);
      setPwdResendTimer(30);
    } catch (err: any) {
      setPwdError(err.response?.data?.detail || 'Failed to dispatch verification code.');
    } finally {
      setPwdLoading(false);
    }
  };
  ...
  const handleConfirmPasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPwdError(err.response?.data?.detail || 'Failed to update password. Invalid verification code.');
    } finally {
      setPwdLoading(false);
    }
  };
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center space-x-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Security & Password Management</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Update your account password with email OTP verification
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-900 hover:bg-cyan-600 dark:bg-slate-800 dark:hover:bg-cyan-600 text-white text-xs font-bold transition-all cursor-pointer shadow-xs self-start sm:self-auto"
          >
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-xs text-slate-500 dark:text-slate-400">
          ...
          <span className="font-mono text-[11px] text-slate-400">
            Linked Email: {accountProfile?.email}
          </span>
        </div>
      </div>
      ...
      {isPasswordModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md animate-fadeIn">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-md w-full p-6 sm:p-8 space-y-6 shadow-2xl">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400">Step {pwdStep} of 2</p>
                </div>
              </div>
              <button
                type="button"
            ...
            {pwdError && (
              <div className="p-3.5 rounded-2xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 text-rose-700 dark:text-rose-400 text-xs flex items-center gap-2.5">
            ...
            {pwdSuccess && (
              <div className="p-3.5 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-400 text-xs flex items-center gap-2.5">
            ...
            {pwdStep === 1 && (
              <div className="space-y-4">
                <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                  For your security, we will send a 6-digit verification code to your registered email address:{' '}
                  <strong className="text-cyan-600 dark:text-cyan-400">{accountProfile?.email}</strong>
                </p>
                ...
                >
                  {pwdLoading ? (
                    ...
                    <>
                      <Send className="h-4 w-4" /> Send Verification Code to Email
                    </>
                  )}
                </button>
              </div>
              ...
              <form onSubmit={handleConfirmPasswordChange} className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <button
                      type="button"
                      disabled={pwdResendTimer > 0 || pwdLoading}
                      onClick={handleRequestPasswordOtp}
                    ...
                    >
                      ...
                      {pwdResendTimer > 0 ? `Resend (${pwdResendTimer}s)` : 'Resend Code'}
                    </button>
                  </div>
                  <input
                    type="text"
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-center text-lg font-bold font-mono tracking-widest text-slate-900 dark:text-white focus:outline-none focus:border-cyan-500"
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl pl-3.5 pr-10 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-cyan-500"
                    />
                    <button
                      type="button"
                ...
                <div className="flex justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <button
                    type="submit"
                    disabled={pwdLoading || pwdOtp.length !== 6 || !pwdNew || !pwdConfirm}
                    className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs shadow-md shadow-emerald-500/20 disabled:opacity-50"
                  >
                    {pwdLoading ? 'Updating...' : 'Confirm & Save Password'}
                  </button>
                </div>
              </form>
            )}
